        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <form method="POST" action="{{ url('foodedit/'.$foodlist->id) }}" enctype="multipart/form-data">
                        @method('PATCH')
                        @csrf
                        <div class="row align-items-start g-3 fs-4">
                            <div class="col-sm-12">
                                <label class="h-100 w-100" for="">ชื่ออาหาร</label>
                                <input class="h-100 w-100 form-control" type="text" name="foodname" value="{{ $foodlist->foodname }}">
                                @error('foodname')
                            ...
                            <div class="col-sm-4">
                                ...
                                <input class="h-100 w-100 form-control" type="number" name="foodkcal" value="{{ $foodlist->foodkcal }}">
                                @error('foodkcal')
                                    <span class="text-danger fs-6">{{ $message }}</span>
                                @enderror
                            </div>
                            <div class="col-4">
                                ...
                                <select class="form-select" aria-label="Default select example" name="foodcategory">
                                    <option selected value="{{ $foodlist->foodcategory }}">{{ $foodlist->foodcategory }}</option>
                            ...
                            <div class="col-6">
                                ...
                                <input class="h-100 w-100 form-control" type="text" name="refer" value="{{ $foodlist->refer }}">
                                @error('refer')
                                    <span class="text-danger fs-6">{{ $message }}</span>
                                @enderror
                            </div>
                            <div class="col-6">
                                ...
                                <input class="h-100 w-100 form-control" type="text" name="video" value="{{ $foodlist->video }}">
                                @error('video')
                                    <span class="text-danger fs-6">{{ $message }}</span>
                                @enderror
                            </div>
                            <div class="col-6">
                                ...
                                <textarea class="w-100 form-control" style="height: 200px; overflow:visible;" type="number" name="foodrecipe">{{ $foodlist->foodrecipe }}</textarea>
                                @error('foodrecipe')
                                    <span class="text-danger fs-6">{{ $message }}</span>
                                @enderror
                            </div>
                            <div class="col-6">
                                ...
                                <textarea class="w-100 form-control" style="height: 200px; overflow:visible;" type="number" name="foodingr">{{ $foodlist->foodingr }}</textarea>
                                @error('foodingr')
                                    <span class="text-danger fs-6">{{ $message }}</span>
                                @enderror
                            </div>
